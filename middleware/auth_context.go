...
func CurrentAccountType(c *gin.Context) string {
	value, _ := c.Get(ContextAccountTypeKey)
	accountType, _ := value.(string)
	if accountType == "" {
		return "user"
	}
	return accountType
...
func CurrentDriverID(c *gin.Context) int64 {
	value, _ := c.Get(ContextDriverIDKey)
	driverID, _ := value.(int64)
	return driverID
}
